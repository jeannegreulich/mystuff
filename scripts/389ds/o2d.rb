...
@slapcat_file = './simp_openldap.ldif'
@ldif_file = './simp_ds389.ldif'
@basedn = ''
...
optparse = OptionParser.new do |opts|
  ...
  opts.on('-f', '--file FILE', 'File that containslapcat data from openldap server', 'Default ./simp_openldap.ldif') do |f|
    @slapcat_file = f.strip
  end
  opts.on('-o', '--output FILE', 'file to output the ldifs to import into ds389 server','Default: ./simp_ds389.ldif') do |f|
